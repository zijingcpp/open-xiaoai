...
基于原始Open-XiaoAI工程对部署的安全性、智能体记忆能力进行了优化。后续功能随着使用还在完善。

## 新增功能

- **多轮连续对话**：LLM 回复播完后自动静默唤醒，无需重复喊"小爱同学"
- **原生 NLP 拦截**：Client 端本地检测并打断小爱原生回复，零网络延迟
- **TTS 分句合并**：可配置分句策略，减少 TTS 合成次数，消除句间空隙
- **智能退出识别**：LLM 通过 `[EXIT]` 标记判断用户退出意图，自动结束对话
- **ASR 防抖**：可配置等待窗口（`asrDebounceMs`），避免话没说完就触发 LLM
- **对话摘要压缩**：历史消息过多时自动摘要，保持上下文连贯
- **持久记忆**：每日自动提取用户偏好和关键事实，跨会话记忆
- **自定义提示音**：多轮对话录音开始时播放自定义提示音
- **调试接口**：`http://127.0.0.1:4400/?text=你好` 可直接发送文本测试
